...
#' @param drop_empty Logical. If TRUE, empty columns will be dropped. Defaults to `FALSE`. This option works only when `preprocess_data = TRUE`.
#' @param preprocess_data Logical. If TRUE, processes data to include value labels and column attributes. Defaults to `FALSE`.
#' @param compact_form Logical. If TRUE, drops expanded checkbox columns, keeping only the main field. This requires `preprocess_data = TRUE`. Defaults to `TRUE`.
#' @param list_event_form Logical. If TRUE, retrieves a mapping of events and forms for longitudinal projects.
...
#' - If `compact_form = TRUE` (requires `preprocess_data = TRUE`), removes expanded checkbox columns while keeping only the main field.
...
#' - If `compact_form = TRUE`, expanded checkbox columns are dropped.
#'
#' @examples
#' \dontrun{
...
#'     drop_empty = TRUE,
#'     compact_form = TRUE,
#'     file_name = "my_redcap_export.rds"
#' )
#'
...
#' - `drop_empty = TRUE` only works when `preprocess_data = TRUE`.
#' - `compact_form = TRUE` removes expanded checkbox columns and requires `preprocess_data = TRUE`.
#'
#' @author
